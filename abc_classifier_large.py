...
class ClassifierEncoder(torch.nn.Module):
    ...
    def __init__(self, vocab_size, dims, layers):
        super().__init__()
...
class ClassifierEncoderTemp(torch.nn.Module):
    """
        IBM Model 2 encoder, modified for classfication.
        A CLS token is added to the input sentence.
        Then classification is based on the value of the CLS position.
...
class ModelTemp(torch.nn.Module):
    ...
    def __init__(self, vocab, d, layers):
        ...
        # so that they get loaded and saved with it.
        self.vocab = vocab
        self.encoder = ClassifierEncoderTemp(len(vocab), 2*d, layers)
...
def loadData(directory):
    ...
    with open(directory + 'train_neg.txt', 'r') as f:
        lines = f.readlines()
        # Process each line
        for line in lines:
            text = line.strip()
    ...
    with open(directory + 'train_pos.txt', 'r') as f:
        lines = f.readlines()
        # Process each line
        for line in lines:
            text = line.strip()
    ...
    with open(directory + 'test_neg.txt', 'r') as f:
        lines = f.readlines()
        # Process each line
        for line in lines:
            text = line.strip()
    ...
    # Read the concatenated file
    with open(directory + 'test_pos.txt', 'r') as f:
        lines = f.readlines()
        # Process each line
        for line in lines:
            text = line.strip()
...
if __name__ == "__main__":
    ...
    small_data_directory = 'formal_language_data/'
    big_data_directory = 'formal_language_data_big/'
    for j in range(2):
        if j == 0:
            data_directory = small_data_directory
            layers = 2
            lr = 0.0001
            epochs = 1
            dims = 5
        else:
            data_directory = big_data_directory
            layers = 4
            lr = 0.0001
            epochs = 50
            dims = 10
        for i in range(4):
            if i <= 1:
                data, test = loadData(data_directory+'abc_noncontiguous/')
                ...
                data, test = loadData(data_directory+'abc/')
                vocab = Vocab()
                # shuffle the data
                for d in data:
                    vocab |= d[1]
            ...
            if i % 2 == 0:
                model = Model(vocab, dims, layers).to(device)
            else:
                model = ModelTemp(vocab, dims, layers).to(device)
            ...
            # check type
            if i == 0:
                torch.save(best_model, 'abc_models/subseq_'+size+"_"+params+'.pt')
            elif i == 1:
                torch.save(best_model, 'abc_models/subseq_temp_'+size+"_"+params+'.pt')
            elif i == 2:
                torch.save(best_model, 'abc_models/substr_'+size+"_"+params+'.pt')
            else:
                torch.save(best_model, 'abc_models/substr_temp_'+size+"_"+params+'.pt')
